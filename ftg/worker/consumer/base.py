...
class PikaConsumer:
    ...
    def setup_exchange(self):
        ...
        """
        log.info("Declaring exchange: %s", self.exchange)
    ...
    def on_exchange_declareok(self, _unused_frame, userdata):
        ...
        for queue_name, kwargs in self.queues.items():
            self.setup_queue(queue_name, **kwargs)

    def setup_queue(self, queue_name, **kwargs):
        """Setup the queue on RabbitMQ by invoking the Queue.Declare RPC
        command. When it is complete, the on_queue_declareok method will
        be invoked by pika.

        :param dict|unicode queue_name: The name of the queue to declare.

        """
        log.info("Declaring queue %s", queue_name)
        cb = functools.partial(self.on_queue_declareok, userdata=queue_name)
        self._channel.queue_declare(queue=queue_name, callback=cb, **kwargs)
    ...
    def on_queue_declareok(self, _unused_frame, userdata):
        ...
        """
        queue_name = userdata
        log.info("Binding %s to %s", self.exchange, queue_name)
    ...
    def on_bindok(self, _unused_frame, userdata):
        ...
        # wait until all queues are binded:
        self._binded_queues.append(userdata)
        if len(self._binded_queues) == len(self.queues):
            self.set_qos()
    ...
    def ack(self, delivery_tag, **kwargs):
        ...
        # it can happen that we want to ack but channel is not up yet
        if self._channel is None:

            def _ack():
                self.ack(delivery_tag, **kwargs)
                self._connection.ioloop.call_later(10, _ack)

            return
        self._channel.basic_ack(delivery_tag, **kwargs)

    def nack(self, delivery_tag, **kwargs):
        ...
        """
        log.warning("NAcknowledging message %s", delivery_tag)
        # it can happen that we want to nack but channel is not up yet
        if self._channel is None:
            # log.warning("Trying to wait for channel to NACK...")
            # time.sleep(5)
            # if not self._consuming or self._channel is None:
            log.error("Cannot NACK because channel (was) down.")
            return
...
class ReconnectingPikaConsumer(object):
    ...
    @property
    def channel(self):
        return self._consumer._channel
