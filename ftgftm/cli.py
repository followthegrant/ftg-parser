...
def split_cois():
    ...
    doc_id, author_name, author_id, coi_statement = [
        str(df.columns[i]) for i in range(4)
    ]
    ...
    df = (
        ...
        .agg(
            {
                **{author_name: tuple, coi_statement: lambda x: " ".join(list(set(x)))},
                **{c: "first" for c in df.columns[3:]},
            }
        )
        .drop_duplicates()
    )
    ...
    for doc_id, row in df.iterrows():
        ...
        authors = list(
            set(
                [
                    tuple(a[0].split(" ", 1)) if " " in a[0] else (None, a[0])
                    for a in row[author_name]
                ]
            )
        )
        explicit_authors = []
