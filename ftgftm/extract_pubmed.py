...
def _(x): return fingerprints.generate(x)  # noqa
...
def make_entities(data, fpath, meta_only):
    ...
    if not meta_only:
        ...
        for key, name in dict(data.get('affiliation_list', [])).items():
            ...
            affiliate.make_id(*sorted(list(set(p for p in str(_(name)).split() if p not in STOPWORDS))))
            if affiliate.id:
                countries = sorted(countrytagger.tag_text_countries(name), key=lambda x: x[1])
                if countries:
        ...
        for last_name, first_name, affiliate_key in authors:
            ...
            # found in this paper
            aff = sorted(a.id for a in author_affiliations[(last_name, first_name)] if a)
            if len(aff):
                    ...
                    for identifier in (author_name, 'all'):
                        if identifier in coi_statements:
                            ...
                            author_coi_article_ref.make_id(
                                'author_coi_article_ref', document_id, author.id, author_coi.id)
